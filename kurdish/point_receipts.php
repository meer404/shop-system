...
if (isset($_GET['view_receipt'])) {
    ...
    if (!$receipt) die("پسوڵە نەدۆزرایەوە!");
    ...
    <head>
      <meta charset="utf-8">
      <title>پسوڵەی خاڵ #<?= safe($receipt['receipt_no']) ?></title>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .totals-section { text-align: left; margin-bottom: 20px; }
        .total-row { display: flex; justify-content: flex-start; padding: 8px 15px; background: #f0f7ff; border: 1px solid #5a88b8; border-radius: 4px; font-size: 1.2em; font-weight: bold; color: #1a365d; }
        .total-label { margin-left: 20px; }
    ...
    <body>
      <div class="receipt-wrapper">
        <div class="header-section">
          <div class="company-header">
            ...
            <div class="bilingual">
              ...
              <div class="company-info"> CONTROL BOARD<br>سلێمانی - چوارباخ خوار فلکەی مامە ڕیشە<br>ژمارەی مۆبایل: 07732828287 - 00722142666<br>ئیمەیڵ: jumaarasoul3@gmail.com</div>
            </div>
             <p>کۆمپانیایی CONTROL BOARD بۆ دروستکردنی بۆردی کارەبایی و دیزاین و جێبەجێکردنی پڕۆژەکان</p>
          </div>
        </div>
        <div class="receipt-details">
          <div class="detail-group"><span class="detail-label">بەروار:</span><span class="detail-value"><?= safe(date('d/m/Y', strtotime($receipt['receipt_date']))) ?></span></div>
          <div class="detail-group"><span class="detail-label">جۆری پسوڵە:</span><span class="detail-value">بە خاڵ</span></div>
          <div class="detail-group"><span class="detail-label">ژمارەی پسوڵە:</span><span class="detail-value"><?= safe($receipt['receipt_no']) ?></span></div>
        </div>
        <div class="customer-section">
          <div class="customer-field"><span class="detail-label">ناوی پسوڵە:</span><span class="detail-value"><?= safe($receipt['receipt_name'] ?? '-') ?></span></div>
          <div class="customer-field"><span class="detail-label">ژمارەی تەلەفۆن:</span><span class="detail-value"><?= safe($receipt['phone'] ?? '-') ?></span></div>
          <div class="customer-field"><span class="detail-label">شوێن:</span><span class="detail-value"><?= safe($receipt['place'] ?? '-') ?></span></div>
        </div>
        <table class="items-table">
            <thead>
              <tr>
                ...
                <th width="30%">کاڵا</th>
                <th width="10%">بڕ</th>
                <th width="13%">خاڵ/یەکە</th>
                <th width="13%">نرخ/خاڵ</th>
                <th width="14%">کۆی خاڵەکان</th>
            ...
            <tbody>
                ...
                <tr>
                  <td><?= $i++ ?></td>
                  <td class="item-name"><?= safe($item['item_name']) ?></td>
                  <td class="number-col"><?= number_format((float)$item['qty'], 2) ?></td>
                  <td class="number-col"><?= number_format((float)$item['item_points'], 2) ?></td>
                  <td class="number-col">$<?= number_format((float)$item['item_price'], 2) ?></td>
                  <td class="number-col"><?= number_format((float)$item['total_points'], 2) ?></td>
                  <td class="number-col"><strong>$<?= number_format((float)$item['line_total'], 2) ?></strong></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <div class="totals-section">
          <div class="total-row">
            ...
            <span class="total-value">$<?= number_format((float)$receipt['grand_total'], 2) ?></span>
          </div>
        </div>
        ...
        <div class="footer-section">
          <div class="print-info">لاپەڕە ١ لە ١ | کات <?= date('d/m/Y h:i:s A') ?> | چاپکراوە لەلایەن: بەڕێوبەر</div>
      ...
      <div class="print-actions">
        ...
        <a href="point_receipts.php">هەموو پسوڵەکانی زەرعە</a>
...
$page_title = "پسوڵەکانی خاڵ";

?>

<div class="card">
    <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        <h2 style="margin:0;">پسوڵەکانی زەرعە </h2>
        <a href="point_receipt_new.php" class="btn">+ پسوڵەی زەعە نوێ</a>
    </div>
    ...
    <form method="get" class="form-row" style="margin:12px 0;">
        <input type="text" name="q" value="<?= safe($q) ?>" placeholder="گەڕان بەدوای ناو، تەلەفۆن، تێبینی...">
        ...
        <a href="point_receipts.php" class="badge">ڕێکخستنەوە</a>
    </form>
</div>
...
<div class="card">
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>#</th><th>ژمارەی پسوڵە</th><th>بەروار</th><th>ناو</th><th>ژمارەی تەلەفۆن</th><th>کۆی گشتی</th><th>کردارەکان</th></tr>
            </thead>
            <tbody>
                <?php if (!$rows): ?>
                    <tr><td colspan="7" style="text-align:center">هیچ پسوڵەیەکی خاڵ نەدۆزرایەوە.</td></tr>
                <?php else: foreach ($rows as $r): ?>
                    <tr>
                        ...
                        <td class="actions" style="display:flex;gap:8px;flex-wrap:wrap;">
                            ...
                            <form method="post" onsubmit="return confirm('دڵنیایت لە سڕینەوەی ئەم پسوڵەیە؟ ئەم کردارە ناگەڕێتەوە.');" style="display:inline; margin:0;">
