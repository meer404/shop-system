<?php $page = 'payments.php'; require_once 'header.php'; require_once '../inc/auth.php'; ?>
<?php
require '../inc/config.php'; $page_title="پارەدانەکان"; $msg=null;
  ...
  <form method="post" action="payments.php">
    <div class="form-row">
      <select name="customer_id" required>
        <option value="">-- كڕیار --</option>
        <?php foreach($customers as $c): ?>
          <option value="<?= (int)$c['id'] ?>" <?= $customer_id_filter==$c['id']?'selected':'' ?>><?= h($c['name']) ?></option>
        <?php endforeach; ?>
      </select>
      <input type="number" step="0.01" name="amount" placeholder="بڕ" required>
      <input name="note" placeholder="تێبینی (ئیختیاری)">
...
<div class="card">
  <h3>پارەدانەکان</h3>
  <table><thead><tr><th>#</th><th>كڕیار</th><th>بڕ</th><th>بەروار</th><th>تێبینی</th></tr></thead><tbody>
