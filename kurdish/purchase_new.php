...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($supplier_id <= 0) {
    ...
    try {
      ...
      $pdo->commit();
      $msg = "<span class='success'>کڕین saved.</span> <a class='badge' href='purchase_receipt.php?id={$last_purchase_id}'>Open receipt</a>";
    } catch (Throwable $e) {
      $pdo->rollBack();
...
function addRow(){
  ...
  tr.innerHTML = `
    ...
    <td>
      <input type="number" step="0.01" name="items[${idx}][sell_price]" value="" placeholder="نرخی فرۆشتن">
    </td>
    <td>
