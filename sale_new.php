<?php $page = 'sale_new.php'; require_once __DIR__ . '/header.php'; ?>
<?php
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $customer_id=(int)($_POST['customer_id']??0); $items=$_POST['items']??[]; $paid=(float)($_POST['paid']??0);
  ...
  elseif(empty($items)){ $msg="Add at least one item."; }
    ...
    foreach($items as $it){
      ...
      $p=$pdo->prepare("SELECT price,stock FROM products WHERE id=?"); $p->execute([$pid]); $pinfo=$p->fetch();
    ...
    if($msg===null){
      ...
      try{
        $pdo->prepare("INSERT INTO sales (customer_id, subtotal, paid, is_credit, note) VALUES (?,?,?,?,?)")->execute([$customer_id,$subtotal,$paid,$is_credit,$note]);
        $sale_id=(int)$pdo->lastInsertId(); $last_sale_id=$sale_id;
        foreach($items as $it){
          $pid=(int)($it['product_id']??0); $qty=(int)($it['qty']??0); if($pid<=0 or $qty<=0) continue;
          $p=$pdo->prepare("SELECT price FROM products WHERE id=?"); $p->execute([$pid]); $price=(float)$p->fetch()['price']; $line=$price*$qty;
          $pdo->prepare("INSERT INTO sale_items (sale_id, product_id, qty, price, line_total) VALUES (?,?,?,?,?)")->execute([$sale_id,$pid,$qty,$price,$line]);
      ...
      }catch(Exception $e){ $pdo->rollBack(); $msg="Error saving sale: ".h($e->getMessage()); }
    }
  }
}
...
function addRow(){
  ...
  const tr=document.createElement('tr'); const idx=tb.children.length;
  tr.innerHTML=`
    <td><select name="items[${idx}][product_id]" onchange="updateRow(this)">
      <option value="">-- Select --</option>
...
// لە کاتی بارکردنی پەڕە یەکجار هەژمار بکە
document.addEventListener('DOMContentLoaded', recalc);


</script>
<div class="form-row">
  <span class="badge"> Total $<strong id="subtotal">0.00</strong></span>
